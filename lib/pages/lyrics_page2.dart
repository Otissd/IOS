...
class _LyricsPagesState2 extends State<LyricsPages2> {
  ...
  Future<void> loadAndParseLyrics() async {
    try {
      final lrcContent =
          await rootBundle.loadString('${widget.song.lyricsFile}');
      setState(() {
  ...
  void fetchLyrics() async {
    try {
      final response = await http.get(
        Uri.parse(
            'http://localhost:8080/api/public/files/download/lrc/1506e862-e344-4eb3-94a5-271dcbc4d1df.lrc'),
      );
      if (response.statusCode == 200) {
        setState(() {
        lyrics = parseLrc(response.body); // Dùng nội dung text từ body
      });
      } else {
